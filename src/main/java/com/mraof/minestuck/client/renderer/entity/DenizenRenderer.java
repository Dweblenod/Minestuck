...
public class DenizenRenderer extends GeoEntityRenderer<DenizenEntity>
{
	...
	public void render(DenizenEntity denizen, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource renderTypeBuffer, int packedLightIn)
	{
		...
		float scaleSwing = (float) (Math.sin(denizen.level.getGameTime() / 50F) + 5); //causes the denizen glow to change in size
		...
		Vec3 denizenHeadVec = denizen.getEyePosition().add(denizen.getLookAngle().scale(5));
		...
		poseStack.translate((angledToCamera.x() * 3) / distanceBetweenEntities, angledToCamera.y() / distanceBetweenEntities + eyeOffset, (angledToCamera.z() * 3) / distanceBetweenEntities);
